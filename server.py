...
def save_room():
    if request.method == "POST":
        data = request.get_json()
        room = data["room"]
        insert_room(room) #Uloteni noce room do db
...
def handle_join_room(data):
    if 'user' in session:  
        ...
        # Remove the disconnected user from all rooms
            ...
            socketio.emit("update_users_list", {'room': room, 'users': users, 'user': session['user']}, room=room)
        print(rooms_users)
# **************************************************************** toto přepisuje to vypsaní co se provede pod disconnect když někdo refreshne stránku
    # emit('update_users_list', {'from': 'join_room','user': session['user'] if room != "manipulatingRoom" else "Nic", 'roomsJoined': rooms_users[room] if room != "manipulatingRoom" else "Nic", 'room':room}, room=room)
...
def handle_leave_room(data):
    ...
    # Remove the disconnected user from all rooms
            ...
            socketio.emit("update_users_list", {'room': room, 'users': users, 'user': session['user']}, room=room)
    print(rooms_users)
    # emit('update_users_list', {'from': 'leave_room', 'user': session['user'] if room != "manipulatingRoom" else "Nic", "room": room}, room=room)
...
def rename_user(data):
    ...
    #     # Kontrola, zda hodnota klíče není prázdná
    #     if rooms_users[room]:
    #         emit('message', {'from': 'rename_user', "room": room}, room= room)
